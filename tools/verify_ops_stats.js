...
/* ──────────────────────────────────────────────────────────────
   9-2. 기기·연령·성별은 '사람 수'다 — 줄 수로 세면 안 된다
        (2026-08-02 사장님 지적: 누적 33명인데 '남성 163' 이 떴다)
   ────────────────────────────────────────────────────────────── */
{
  console.log("\n■ 9-2. 연령·성별·기기는 사람 수로 센다");
  const rows = [];
  // 부지런한 40대 남성 한 명이 오늘 50번 눌렀다
  for (let i = 0; i < 50; i++)
    rows.push(row(T(3600000 + i * 1000), "busy", i ? "feature" : "visit", "f" + i,
                  { dev: "iOS", age: "40대", gen: "남성" }));
  // 한 번만 온 30대 여성 두 명
  rows.push(row(T(7200000), "quietA", "visit", "", { dev: "PC", age: "30대", gen: "여성" }));
  rows.push(row(T(7201000), "quietB", "visit", "", { dev: "PC", age: "30대", gen: "여성" }));
  // 동의하지 않은 사람 — 어느 칸에도 안 들어간다
  rows.push(row(T(7300000), "noprofile", "visit", "", { dev: "Android" }));

  const s = build(rows).sum();
  const get = (list, k) => (list.find((x) => x[0] === k) || [k, 0])[1];

  eq(get(s.genders, "남성"), 1, "50번 누른 한 사람은 '남성 1' — 50 이 아니다");
  eq(get(s.genders, "여성"), 2, "여성 2명");
  eq(get(s.ages, "40대"), 1, "40대 1명");
  eq(get(s.ages, "30대"), 2, "30대 2명");
  eq(get(s.devices, "iOS"), 1, "기기도 사람 수 — iOS 1명");
  eq(get(s.devices, "PC"), 2, "PC 2명");

  const people = s.genders.reduce((a, x) => a + x[1], 0);
  ok(people <= s.uniq, "성별 합(" + people + ")이 누적 사용자(" + s.uniq + ")를 넘지 않는다");
  eq(s.counts, "people", "화면이 '명'이라고 적어도 되는지 알려주는 표시");

  // 오늘 칸도 같은 규칙이어야 한다 — 한쪽만 고치면 오늘/누적을 오갈 때 숫자가 튄다
  eq(get(s.today.genders, "남성"), 1, "오늘 칸도 사람 수");
  eq(get(s.today.ages, "40대"), 1, "오늘 연령도 사람 수");

  // 골프장·기능 순위는 '몇 번 눌렸나' 라서 그대로 줄 수여야 한다
  eq(get(s.features, "f1"), 1, "기능 순위는 눌린 횟수 그대로");
  eq(s.today.hits, 4, "방문 수는 visit 줄 수 그대로 (네 사람이 한 번씩 열었다)");
}
