...
현재 웨지 아이콘은 `tools/make_club_icons_src.py` 가 `assets/src/wedge.jpg` 로 만듭니다.
이 파일은 그때 왜 이렇게 했는지 기록으로만 남겨 둡니다. **실행하지 마세요.**
(같은 이유로 `tools/build_club_icons.js` 도 쓰지 않습니다 — 둘 다 assets/*.png 를 덮어씁니다.)
