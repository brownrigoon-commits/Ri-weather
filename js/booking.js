...
/* ───────── 폰에서 어떻게 열리는가 — 2026-08-03 실측, 다시 재지 말 것 ─────────
 *
 * 사장님 지적: "폰으로 PC 화면이 나오면 작동 못합니다" (한국 골팡에서 겪은 그 문제).
 * 확인 방법: UA 를 아이폰으로 놓고 같은 주소를 받아 viewport 메타를 봤다.
 *
 *  ① 화면 — 둘 다 **주소는 하나인데 UA 를 보고 다른 HTML 을 준다**. 안전하다.
 *       golf-jalan.net        폰 → width=device-width  / PC → viewport 없음
 *       search.gora…/cal/disp 폰 → width=device-width  / PC → viewport 없음
 *     PC용·모바일용 주소가 갈려 있지 않아 골팡 같은 사고가 구조적으로 안 난다.
 *
 *  ② 앱 자동 열기(유니버설 링크) — 각 도메인의 /.well-known/ 규약 파일로 확인했다.
 *       じゃらんゴルフ ✅ 앱 jp.co.recruit.jalangolf 이 "슬래시 gc 별표 슬래시" 경로를 등록해 뒀다.
 *                       (별표를 그대로 적으면 이 주석이 거기서 닫힌다 — 그래서 말로 적는다)
 *                       → 우리 주소가 여기 걸린다. **앱이 깔렸으면 앱이 열리고 날짜도 넘어간다.**
 *                          우리가 할 일이 없다. 자체 스킴을 쓰면 오히려 앱 없는 폰에서 깨진다.
 *       楽天GORA      ❌ search·gora 두 도메인 모두 규약 파일이 404 다. 웹으로만 열린다.
 *                       gora:// 같은 자체 스킴은 앱 없는 폰에서 아무 일도 안 일어난다 → 안 쓴다.
 *       hb.afl(제휴)  ⚠️ 규약은 있으나 등록 경로가 /ichiba/* · /fashion/* · /travel/* 뿐이다.
 *                       우리가 쓰는 /hgc/ 는 없다 → 라쿠텐 쇼핑앱이 채가지 않는다(다행).
 *                       제휴 리다이렉트를 지나도 month·day 파라미터가 그대로 남는 것도 확인했다.
 * ──────────────────────────────────────────────────────────────────── */
