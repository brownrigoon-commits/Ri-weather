...
## ⏳ 대기 중: 백엔드 v5 재배포 (1분, 구글 로그인 필요)

`tools/apps_script/Code.gs` 최신본(사진을 '리뷰 사진만'으로 서버에서 선별 + 캐시키 p3)이
아직 Apps Script에 반영되지 않았다. 현재는 앱 쪽 이중 필터가 막고 있어 **틀린 사진은 안 나가지만**,
서버가 블로그 썸네일 섞인 10장을 주고 앱이 걸러내는 구조라 **가게당 사진 장수가 적다**(평균 1~2장).
재배포하면 리뷰 사진만 10장 채워져 온다.
방법: script.google.com 로그인 → '골프라이프 백엔드' → 코드 붙여넣기(전체 교체) → 저장 →
배포 관리 → ✏️ → 버전 '새 버전' → 배포. (URL 불변)
