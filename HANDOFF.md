# 골프라이프 작업 이어하기 가이드 (2026-07-27 새벽 갱신)
...
# 📈 Ri_Stock — 주식 앱 (2026-07-27 신규, **사장님 확인 필요**)

> 이 저장소에 앱이 하나 더 생겼습니다. 골프앱은 그대로 있고, 파일이 겹치지 않습니다.
> 지금 **작업 가지(브랜치) `claude/ri-stock-automation-cbi0pc` 에 있고 아직 main 이 아닙니다.**
...
### 1) main 에 합치기 (제일 먼저)
가지 `claude/ri-stock-automation-cbi0pc` 를 main 에 합칩니다.
**깃허브의 예약 실행은 main 에 있는 것만 동작합니다.** 합치기 전엔 자동으로 아무 일도 일어나지 않습니다.
...
## 🟢 회사에서 처음 여시면 — 여기부터 (2026-07-27)

```
python tools/sync.py          ← 집에서 한 작업 받기 (v124)
```

**앱 이름이 Ri-Weather → 골프라이프 로 바뀌었습니다.** 저장소·URL은 그대로입니다.
집에서 v110 → **v124** 까지 15번 배포했고 전부 검증 통과했습니다.

### 무엇이 달라졌나 (한눈에)

| 영역 | 내용 |
|---|---|
| **디자인** | 다크 → **토스식 라이트 테마** 전면 개편. 포인트색 = 로고 그린, 서브 = 깃발 빨강 |
| **날씨** | 아이폰 날씨 앱처럼 카드가 하늘이 됨. 비·눈은 **캔버스 입자**(`js/weatherfx.js`) |
| **대기 화면** | 스피너 대신 **원형 % + 말 거는 문구** (`js/loading.js`) |
| **맛집** | 거리순 먼저 → 재정렬 방식 폐기. **다 모아서 한 번에** 추천순 표시 |
| **클럽 피팅** | 드라이버 + **아이언·웨지·퍼터** 추가. 선호 브랜드 1순위 + 타 브랜드 대안 |
| **숙박** | 신규 메뉴. 골프장 기준 주변 숙소 (`js/stay.js`) |
| **백엔드** | 백업·복구가 **배포된 적 없던 것**을 찾아 버전 8로 재배포. 전 과정 검증 완료 |

### 새로 생긴 파일
```
js/weatherfx.js   비·눈 캔버스 효과       js/stay.js    숙박 메뉴
assets/golfer.png 피팅 캐릭터(사장님 제공)  tools/sweep.js 자동 검사 도구
```

### ⚠️ 회사에서 작업 전 꼭 알아둘 것

1. **배포는 두 단계다** — `release_courses.py` → `verify_deploy.py --wait` 통과해야 '완료'
2. **Apps Script 는 저장만으로 반영 안 된다** — 배포 관리 → **기존 배포 수정** → 새 버전
   (새 배포를 만들면 URL 이 바뀌어 앱이 끊긴다. 이것 때문에 백업 기능이 3일간 죽어 있었다)
3. **검사는 자가검증하고 쓸 것** — `tools/sweep.js` 를 브라우저 콘솔에 붙여 실행.
   일부러 고장을 내서 잡히는지 확인 안 하면 통과가 무의미하다
   (실제로 app.js 가 통째로 죽었는데 스윕이 통과한 적 있음)
4. **색 역할을 합치지 말 것** — `--brand`(밝은 로고 그린, 표시용) / `--primary`(진한 그린, 흰 글씨 버튼용)
   / `--primary-ink`(흰 배경 위 글씨) / `--flag`(빨강, 핀·오버파) / `--rain`(파랑, 강수 데이터)

### 사장님이 내일 확인하실 것
비 그래픽 느낌 · 클럽 피팅 브랜드 추천 · 숙박 메뉴. 피드백 오면 이어서 작업.
