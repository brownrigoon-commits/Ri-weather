...
- 수집: `tools/jp/pgm_collect.py` + `tools/jp/accordia_collect.py`.
  **2026-07-31 Phase 0 실측으로 확정된 사실**(설계 초안의 추정을 아래로 대체):
  ...
  | | PGM (~150곳) | 아코디아 (172곳) |
  |---|---|---|
  | 코스가이드 URL | `pacificgolf.co.jp/{슬러그}/**course.asp**` | `reserve.accordiagolf.com/golfCourse/{도도부현}/{슬러그}/**layout**` |
  | 홀별 파·HDCP | ✅ | ✅ |
  | 티별 야드 | ✅ Black/Blue/White/Red | ✅ Blue/White/Green/Red |
  | 홀맵 | ✅ **SVG** `/{슬러그}/assets/img/course/{n}.svg` | ✅ **JPG** `/images/course/{구장코드}/courseLayout/{나인}_{홀}.jpg` |
  | 홀별 공략문 | **구장마다 다름** (남이치하라 O / 마리아·무사시 X) | ❌ 없음 |
  | 전 구장 인덱스 | ✅ **`sitemap.xml`** (400+ URL, 슬러그 다수 확인) | 미확인 — 현별 목록 페이지에서 추출 |

  - ⚠️ **`course_plain.asp` 는 표준이 아니다** — 남이치하라만 그렇고 마리아는 404다.
    `course.asp` 를 기본으로 하고 실패하면 변형을 시도할 것.
  - ⚠️ 아코디아 이미지의 **절대 호스트는 미검증**(경로가 상대경로였다).
    `reserve.accordiagolf.com` / `www.accordiagolf.com` 두 곳을 실제로 받아 보고 정할 것.
