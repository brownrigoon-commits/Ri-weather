...
| 단계 | 상태 |
|---|---|
| **A. 홈 화면 앱 완성도 (아이폰)** | ✅ **완료** — 사장님 요청으로 아이폰만 먼저 만들었습니다 |
| **B. 안드로이드 APK (TWA)** | ⬜ 설계만. 안드로이드 폰을 쓰는 분이 생기면 그때 (§3) |
| **C. 스토어 등록** | ⬜ 비용·심사. 사장님 결정 대기 (§7) |

A단계에서 설계와 **달라진 두 가지**(둘 다 실물을 보고 판단):
- 스플래시를 6종이 아니라 **8종** 만들었습니다. 아이폰은 해상도가 정확히 맞아야만 쓰는데,
  6종만으로는 iPhone 14 Pro(1179×2556)·XS Max(1242×2688)를 쓰는 분이 흰 화면을 봅니다.
  단색 배경 PNG 라 8장 합쳐도 253KB 로 예산(300KB) 안입니다.
- manifest `screenshots` 는 **넣지 않았습니다.** iOS 가 읽지 않는 필드입니다
  (안드로이드 설치 시트용) — B단계로 미뤘습니다.
