  ...
  function 브리핑다시받기(버튼) {
    ...
    fetch(백엔드, {
      ...
      .then(function (답) {
        ...
        // 옛 백엔드는 이 요청을 모른 채 `{ok:false}` 만 돌려줍니다(사유 칸이 없습니다).
        // '알 수 없는 오류' 라고 하면 무엇을 해야 할지 알 수 없으므로 그 경우를 따로 구분합니다.
        var 사유 = (답 && 답.사유) || (답 && 답.ok === false ? '서버미갱신' : '연결실패');
        var 말 = {
          잠시전실행: '방금 한 번 돌렸습니다. 3분 뒤에 다시 눌러 주세요.',
          토큰없음: '서버에 깃허브 토큰이 아직 없습니다. 최신 데이터만 다시 불러왔습니다.',
          서버미갱신: '백엔드를 아직 새 버전으로 배포하지 않았습니다. 최신 데이터만 다시 불러왔습니다.',
          연결실패: '서버에 닿지 못해 최신 데이터만 다시 불러왔습니다.'
        }[사유] || ('재수집 요청이 닿지 않아(' + 사유 + ') 최신 데이터만 다시 불러왔습니다.');
