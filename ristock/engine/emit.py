...
def 기준선시장기준일(기준선, 시장):
    ...
    표 = (기준선 or {}).get('시장기준일') or {}
    return str(표.get(시장) or (기준선 or {}).get('기준일') or '')
...
def _선정병합(바탕, 새것):
    ...
    """
    합 = {전략id: dict(시장별 or {}) for 전략id, 시장별 in (바탕 or {}).items()}
    for 전략id, 시장별 in (새것 or {}).items():
        합.setdefault(전략id, {}).update(시장별 or {})
    return 합
...
def 기준선승격(기준선, 기준일):
    ...
    시장기준일 = dict(기준선.get('시장기준일') or {})
    시장기준일.update(대기.get('시장기준일') or {})
